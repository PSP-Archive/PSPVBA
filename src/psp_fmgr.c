...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if ((!strcasecmp(pszExt, ".rom"))||
        ...
        (!strcasecmp(pszExt, ".mb" ))||
        (!strcasecmp(pszExt, ".agb"))||
        (!strcasecmp(pszExt, ".gba"))) {
...
static char user_filedir_roms[PSP_FMGR_MAX_NAME];
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_ROM) /* Disk */ error = gba_load_rom(user_filename);
      }
      else 
      {
        if (file_format == FMGR_FORMAT_ROM) /* Rom */ error = gba_load_rom(user_filename);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = gba_load_file_settings(user_filename);
      }
    }
