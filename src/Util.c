...
bool utilWriteBMPFile(const char *fileName, int w, int h, u8 *pix)
{
  ...
  utilPutWord(bmpheader.planes, 1);
  utilPutWord(bmpheader.bitsperpixel, 24);
...
static u8 *utilLoadFromZip_ref(const char *file,
...
static u8 *utilLoadGzipFile_ref(const char *file,
                            bool (*accept)(const char *),
                            u8 *data,
                            int *size)
{
...
{
  if(utilIsZipFile(file)) {
    return utilLoadFromZip_ref(file, accept, data, size);
  }
  if(utilIsGzipFile(file)) {
    return utilLoadGzipFile_ref(file, accept, data, size);
  }
