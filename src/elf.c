  ...
  while(data < end && state->pc < pc) {
    ...
    case 0:
      ...
      case DW_CFA_remember_state:
        fs = (ELFFrameStateRegisters *)calloc(1, sizeof(ELFFrameStateRegisters));
...
u32 elfDecodeLocation_pfunc_pblk_ploc_u32(Function *f, ELFBlock *o, enum LocationType *type, u32 base)
{
  u32 framebase = 0;
  if(f && f->frameBase) {
...
u32 elfDecodeLocation_pfunc_pblk_ploc(Function *f, ELFBlock *o, enum LocationType *type)
{
  return elfDecodeLocation_pfunc_pblk_ploc_u32(f, o, type, 0);
}
...
u8 *elfReadAttribute(u8 *data, ELFAttr *attr)
{
 ...
 start:
  switch(form) {
  case DW_FORM_addr:
    attr->u.value = elfRead4Bytes(data);
    data += 4;
    break;
  case DW_FORM_data2:
    attr->u.value = elfRead2Bytes(data);
    data += 2;
    break;
  case DW_FORM_data4:
    attr->u.value = elfRead4Bytes(data);
    ...
    break;
  case DW_FORM_string:
    attr->u.string = (char *)data;
    data += strlen(attr->u.string)+1;
    break;
  case DW_FORM_strp:
    attr->u.string = elfDebugStrings + elfRead4Bytes(data);
    data += 4;
    break;
  case DW_FORM_block:
    attr->u.block = (ELFBlock *)malloc(sizeof(ELFBlock));
    attr->u.block->length = elfReadLEB128(data, &bytes);
    data += bytes;
    attr->u.block->data = data;
    data += attr->u.block->length;
    break;
  case DW_FORM_block1:
    attr->u.block = (ELFBlock *)malloc(sizeof(ELFBlock));
    attr->u.block->length = *data++;
    attr->u.block->data = data;
    data += attr->u.block->length;
    break;
  case DW_FORM_data1:
    ...
    break;
  case DW_FORM_flag:
    attr->u.flag = (*data++) ? true : false;
    break;
  case DW_FORM_sdata:
    attr->u.value = elfReadSignedLEB128(data, &bytes);
    data += bytes;
    break;
  case DW_FORM_udata:
    attr->u.value = elfReadLEB128(data, &bytes);
    data += bytes;
    break;
  case DW_FORM_ref_addr:
    attr->u.value = (elfDebugInfo->infodata + elfRead4Bytes(data)) -
  ...
  case DW_FORM_ref_udata:
    attr->u.value = (elfDebugInfo->infodata +
                   (elfGetCompileUnitForData(data)->top -
...
void elfParseType_pu8_pabb_pcom_pptype(u8 *data, u32 offset, ELFAbbrev *abbrev, CompileUnit *unit,
                  Type **type)
{
  switch(abbrev->tag) {
  case DW_TAG_typedef:
    {
      ...
      *type = elfParseType_punit_u32(unit, typeref);
      if(name)
        (*type)->name = name;
      return;
    }
          ...
          case DW_TAG_member:
            {
              ...
              for(int i = 0; i < abbr->numAttrs; i++) {
                ...
                switch(attr->name) {
                case DW_AT_name:
                  m->name = attr->u.string;
                  break;
                case DW_AT_type:
                  m->type = elfParseType_punit_u32(unit, attr->u.value);
                  break;
                case DW_AT_data_member_location:
                  m->location = attr->u.block;
                  break;
                case DW_AT_byte_size:
                  m->byteSize = attr->u.value;
                  break;
                case DW_AT_bit_offset:
                  m->bitOffset = attr->u.value;
                  break;
                case DW_AT_bit_size:
                  m->bitSize = attr->u.value;
                  break;
                case DW_AT_decl_file:
                case DW_AT_decl_line:
  ...
  case DW_TAG_base_type:
    {
      ...
      for(int i = 0; i < abbrev->numAttrs; i++) {
        ...
        case DW_AT_name:
          ...
          break;
        case DW_AT_encoding:
          t->encoding = attr->u.value;
          break;
        case DW_AT_byte_size:
          t->size = attr->u.value;
          break;
        case DW_AT_bit_size:
          t->bitSize = attr->u.value;
          break;
        default:
  ...
  case DW_TAG_pointer_type:
    {
      ...
      for(int i = 0; i < abbrev->numAttrs; i++) {
        ...
        switch(attr->name) {
        case DW_AT_type:
          t->u.pointer = elfParseType_punit_u32(unit, attr->u.value);
          break;
        case DW_AT_byte_size:
          t->size = attr->u.value;
          break;
  ...
  case DW_TAG_subroutine_type:
    {
      ...
      for(int i = 0; i < abbrev->numAttrs; i++) {
        ...
        case DW_AT_sibling:          
          break;
        case DW_AT_type:
          f->returnType = elfParseType_punit_u32(unit, attr->u.value);
          break;
        default:
  ...
  case DW_TAG_array_type:
    {
      ...
      for(i = 0; i < abbrev->numAttrs; i++) {
        ...
        case DW_AT_type:
          typeref = attr->u.value;
          array->type = elfParseType_punit_u32(unit, typeref);
          break;
        default:
          ...
          case DW_TAG_subrange_type:
            {
              ...
              for(int i = 0; i < abbr->numAttrs; i++) {
                ...
                case DW_AT_upper_bound:
                  array->bounds[index] = attr->u.value+1;
                  break;
...
Type *elfParseType_punit_u32(CompileUnit *unit, u32 offset)
{
  Type *t = unit->types;
  ...
  elfParseType_pu8_pabb_pcom_pptype(data, offset, abbrev, unit, &type);
  return type;
}
...
void elfGetObjectAttributes(CompileUnit *unit, u32 offset, Object *o)
{
  ...
  for(int i = 0; i < abbrev->numAttrs; i++) {
    ...
    switch(attr->name) {
    case DW_AT_location:
      o->location = attr->u.block;
      break;
    case DW_AT_name:
      if(o->name == NULL)
        o->name = attr->u.string;
      break;
    ...
    case DW_AT_decl_line:
      ...
      break;
    case DW_AT_type:
      o->type = elfParseType_punit_u32(unit, attr->u.value);
      break;
    case DW_AT_external:
      o->external = attr->u.flag;
      break;
    case DW_AT_const_value:      
                   ...
                   Object **object)
{
  ...
  for(int i = 0; i < abbrev->numAttrs; i++) {
    ...
    case DW_AT_external:
      ...
      break;
    case DW_AT_abstract_origin:
      elfGetObjectAttributes(unit, attr->u.value, o);
      break;
    case DW_AT_const_value:      
    case DW_AT_declaration:
    case DW_AT_artificial:
...
void elfGetFunctionAttributes(CompileUnit *unit, u32 offset, Function *func)
{
  ...
  for(int i = 0; i < abbrev->numAttrs; i++) {
    ...
    case DW_AT_name:
      if(func->name == NULL)
        func->name = attr->u.string;
      break;
    case DW_AT_MIPS_linkage_name:
      func->name = attr->u.string;
      break;
    case DW_AT_low_pc:
      func->lowPC = attr->u.value;
      break;
    case DW_AT_high_pc:
      func->highPC = attr->u.value;
      break;
    case DW_AT_decl_file:
      func->file = attr->u.value;
      break;
    case DW_AT_decl_line:
      func->line = attr->u.value;
      break;
    case DW_AT_external:
      func->external = attr->u.flag;
      break;
    case DW_AT_frame_base:
      func->frameBase = attr->u.block;
      break;
    case DW_AT_type:
      func->returnType = elfParseType_punit_u32(unit, attr->u.value);
      break;
    case DW_AT_inline:
    case DW_AT_specification:
    case DW_AT_declaration:
  ...
  *f = func;
  ...
  for(int i = 0; i < abbrev->numAttrs; i++) {
    ...
    case DW_AT_type:
      ...
      break;
    case DW_AT_abstract_origin:
      elfGetFunctionAttributes(unit, attr->u.value, func);
      break;
    case DW_AT_declaration:
      declaration = attr->u.flag;
      break;
    case DW_AT_inline:
    case DW_AT_specification:
    case DW_AT_artificial:
...
CompileUnit *elfParseCompUnit(u8 *data, u8 *abbrevData)
{
  ...
  for(i = 0; i < abbrev->numAttrs; i++) {
    ...
    switch(attr->name) {
    case DW_AT_name:
      unit->name = attr->u.string;
      break;
    case DW_AT_stmt_list:
      unit->hasLineInfo = true;
      unit->lineInfo = attr->u.value;
      break;
    case DW_AT_low_pc:
      unit->lowPC = attr->u.value;
      break;
    case DW_AT_high_pc:
      unit->highPC = attr->u.value;
      break;
    case DW_AT_compdir:
      unit->compdir = attr->u.string;
      break;
      // ignore
    case DW_AT_language:
    case DW_AT_producer:
...
bool elfReadProgram_ref(ELFHeader *eh, u8 *data, int* size, bool parseDebug)
{
...
bool elfRead_ref(const char *name, int* siz, FILE *f)
{
  fseek(f, 0, SEEK_END);
  long size = ftell(f);
  ...
  if(!elfReadProgram_ref(header, elfFileData, siz, parseDebug)) {
    free(elfFileData);
    elfFileData = NULL;
    return false;
  }
...
void elfCleanUp_pobj(Object *o)
{
  free(o->location);
}

void elfCleanUp_pfunc(Function *func)
{
...
void elfCleanUp_ppabb(ELFAbbrev **abbrevs)
{
  for(int i = 0; i < 121; i++) {
...
void elfCleanUp_ptype(Type *t)
{
  switch(t->type) {
  case TYPE_function:
    if(t->u.function) {
      Object *o = t->u.function->args;
      while(o) {
        elfCleanUp_pobj(o);
        Object *next = o->next;
        free(o);
  ...
  case TYPE_union:
    if(t->u.structure) {
      for(int i = 0; i < t->u.structure->memberCount; i++) {
        free(t->u.structure->members[i].location);
      }
      free(t->u.structure->members);
      free(t->u.structure);
    }
    break;
  case TYPE_enum:
    if(t->u.enumeration) {
      free(t->u.enumeration->members);
      free(t->u.enumeration);
    }
    break;
  case TYPE_base:
  case TYPE_pointer:
...
void elfCleanUp_pcomp(CompileUnit *comp)
{
  elfCleanUp_ppabb(comp->abbrevs);
  free(comp->abbrevs);
...
void elfCleanUp_void()
{
  CompileUnit *comp = elfCompileUnits;

  while(comp) {
    elfCleanUp_pcomp(comp);
